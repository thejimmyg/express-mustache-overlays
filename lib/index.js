...
const setupMustacheOverlays = async (app, options) => {
  let { mustacheDirs = [], publicFilesDirs = [], expressStaticOptions, title='Express Mustache Overlays', scriptName = '', publicURLPath, ...rest } = options || {}
  if (!publicURLPath) {
  ...
  app.use((req, res, next) => {
    res.locals = Object.assign({}, res.locals, { publicURLPath, scriptName, title })
    next()
  })
