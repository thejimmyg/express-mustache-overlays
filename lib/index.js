...
const mustacheFromEnv = (app) => {
  ...
  debug('MUSTACHE_DIRS environment variable directories:', dirs)
  return dirs
}
...
const prepareMustache = (app, userDirs) => {
  app.locals.mustache = {
    userDirs: userDirs || [],
    libDirs: [],
    overlay: (dirs) => {
      ...
      for (let i = 0; i < dirs.length; i++) {
        ...
        accessAsync(dir, fs.constants.R_OK).catch((e) => {
          const msg = `WARNING: Cannot access ${dir}: ${e}`
          console.error(msg)
          debug(msg)
          return true
        })
        debug(`Overlaying the directory ${dir} into app.locals.mustache.libDirs`)
